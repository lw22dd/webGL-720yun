...
import (
	...
	"webGL-720yun/internal/upload/service"
...
func NewUploadHandler(uploadService *service.UploadService, wsHub *websocket.Hub, jwtService *jwt.JWTService) *UploadHandler {
...
func InitUpload(svc *service.UploadService) gin.HandlerFunc {
...
func UploadChunk(svc *service.UploadService) gin.HandlerFunc {
	return func(c *gin.Context) {
		userID, _, _ := middleware.GetCurrentUser(c)
...
func MergeChunks(svc *service.UploadService) gin.HandlerFunc {
	return func(c *gin.Context) {
		userID, _, _ := middleware.GetCurrentUser(c)

		var req dto.MergeUploadRequest
		if err := c.ShouldBindJSON(&req); err != nil {
			utils.BadRequest(c.Writer, "请求参数错误: "+err.Error())
			return
		}
...
func GetUploadStatus(svc *service.UploadService) gin.HandlerFunc {
	return func(c *gin.Context) {
		userID, _, _ := middleware.GetCurrentUser(c)
...
func CancelUpload(svc *service.UploadService) gin.HandlerFunc {
	return func(c *gin.Context) {
		userID, _, _ := middleware.GetCurrentUser(c)
